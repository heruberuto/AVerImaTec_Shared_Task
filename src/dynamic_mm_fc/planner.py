...
class Plan_Model:
                    ...
                    for ques in questions:
                        ...
                        if "Image-related" in ques["question_type"]:
                            ...
                            if "Image" in [ans["answer_type"] for ans in answers]:
                                ...
                                plan = "C"
                        all_data_corpus.append({"question": ques_txt, "plan": plan, "ques_type": ques_type})
    ...
    def gen_next_plan_zero(self, ques, ques_type):
        if "gemini" in self.plan_llm_name:
            ...
            response = self.plan_llm.models.generate_content(model=self.plan_llm_name, contents=inputs)
            ...
            model_inputs = self.plan_llm["tokenizer"]([text], return_tensors="pt").to(
                self.plan_llm["model"].device
            )
            generated_ids = self.plan_llm["model"].generate(**model_inputs, max_new_tokens=256)
            generated_ids = [
