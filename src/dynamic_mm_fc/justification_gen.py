...
class Justification_Model:
    ...
    def justi_gen(self, cur_verdict, meta_info, claim_txt, claim_img, evid_context, num_orig_claim_imgs):
        if "gemini" in self.verify_mllm_name:
            ...
            inputs.append(
                (
                    "\nThe predicted verdict is %s. Please generate your justification (i.e., explanation) for the verdict:"
                )
                % cur_verdict
            )
            response = self.verify_mllm.models.generate_content(model=self.verify_mllm_name, contents=inputs)
            ...
            messages = [
                {
                    "role": "user",
                    "content": [{"type": "text", "text": (self.prompt_temp % (meta_info, claim_txt))}],
                }
            ]
                            ...
                            if sp_str in img_token_list:
                                ...
                                try:
                                    messages[0]["content"].append(
                                        {"type": "image", "image": evid_images[int(img_idx) - 1]}
                                    )
                                except:
                                    continue
                                #    num_images+=1
            ...
            prompt += evid_texts
            prompt += "\nVerdict:"
            inputs = self.verify_mllm["processor"](images=[image_list], text=prompt, return_tensors="pt").to(
                self.verify_mllm["model"].device
            )
            output = self.verify_mllm["model"].generate(**inputs, max_new_tokens=1024)
            ques = self.verify_mllm["processor"].decode(output[0], skip_special_tokens=True)[
