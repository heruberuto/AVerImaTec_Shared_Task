...
class QG_Model:
                    ...
                    for ques in questions:
                        ...
                        for ans in answers:
                            if ans["answer_type"] == "Image":
                                ...
                                ans_txt = ", ".join(
                                    ["[IMG_" + str(3 + k) + "]" for k in range(len(image_answers))]
                                )
                        ...
                        if "Image-related" in ques["question_type"] and len(ques["input_images"]) > 0:
                            ...
                            img_idx = (
                                "**Image Index:** "
                                + ",".join([str(i + 1) for i, _ in enumerate(ques["input_images"])])
                                + "."
                            )
                        else:
                            q_t = "**Text-related:**"
                        ...
                        question_info_pro.append(
                            {"question": ques_txt, "ques_type": q_t, "answers": ans_info, "img_idx": img_idx}
                        )
                    all_data_corpus.append({"claim": claim_txt, "ques_info": question_info_pro})
    ...
    def gen_initial_ques(self, claim_txt, claim_img):
        if "gemini" in self.qg_model_name:
            ...
            response = self.qg_model.models.generate_content(model=self.qg_model_name, contents=inputs)
            ques = response.text
        elif self.qg_model_name in ["qwen", "gemma"]:
            messages = [
                {
                    "role": "user",
                    "content": [{"type": "text", "text": (self.init_temp["first"] % claim_txt)}],
                }
            ]
            for img in claim_img:
            ...
            if self.qg_model_name == "llava":
                ...
                output = self.qg_model["model"].generate(
                    **inputs, max_new_tokens=128, do_sample=True, temperature=0.5
                )
                ques = (
                    self.qg_model["processor"]
                    .decode(output[0], skip_special_tokens=True, clean_up_tokenization_spaces=False)
            ...
            """
            prompt = (
                self.init_temp % claim_txt + "The image index should be smaller than " + str(len(claim_img))
            )
            image_list = []
            """
            for img in claim_img:
            ...
            inputs = (
                self.qg_model["processor"](images=image_list, text=prompt, return_tensors="pt")
                .to(torch.bfloat16)
                .to(self.qg_model["model"].device)
            )
            output = self.qg_model["model"].generate(
            ...
            ques = self.qg_model["processor"].decode(output[0], skip_special_tokens=True)[
    ...
    def gen_follow_ques(self, claim_txt, claim_img, evid_context, meta_date, meta_loc, num_orig_claim_imgs):
        if "gemini" in self.qg_model_name:
            ...
            messages = [
                {
                    "role": "user",
                    "content": [{"type": "text", "text": (self.follow_temp["first"] % claim_txt)}],
                }
            ]
            for img in claim_img[:num_orig_claim_imgs]:
                messages[0]["content"].append({"type": "image", "image": img})
            ...
            for img in claim_img[:num_orig_claim_imgs]:
                im = Image.open(img)
                image_list.append(im)
            inputs = self.qg_model["processor"](images=[image_list], text=prompt, return_tensors="pt").to(
                self.qg_model["model"].device
            )
            output = self.qg_model["model"].generate(
                **inputs, max_new_tokens=128, do_sample=True, temperature=0.5
            )
    ...
    def generate_icl_texts(self, docs, len_evid):
        ...
        for doc in docs:
            ...
            if len_evid == 0:
                ...
                ques = valid_q[0]
                texts.append(
                    "Claim: %s Question: %s Image index: %s"
                ...
                for i, ques in enumerate(question_info):
                    ...
                    if i == 0:
                        if img_idx is not None:
                            question += ", ".join(
                                ["[IMG_" + str(1 + k) + "]" for k in range(len(img_idx.split(",")))]
                            )
                        evid_history = qa_to_evidence.qa_to_evid(
                            question, answers, self.answer_llm, self.answer_llm_name, self.path
                        )
                    elif i == 1:
                ...
                texts.append(
                    "Claim: %s Evidence history: %s Generated questions: %s"
                    % (claim_txt, evid_history, cur_info)
                )
        texts = "\n".join(texts)
        print("ICL QG:\n", texts)
        return texts

    def gen_ques_few_shot(
        self, claim_txt, claim_img, evid_context, meta_date, meta_loc, num_orig_claim_imgs, num_demo=2
    ):
        ...
        if "gemini" in self.qg_model_name:
            ...
            for img in claim_img[:num_orig_claim_imgs]:
                messages[0]["content"].append({"type": "image", "image": img})
            if len(evid_context) == 0:
                messages[0]["content"].append({"type": "text", "text": (self.icl_temp["init_ques"])})
            else:
                messages[0]["content"].append({"type": "text", "text": (self.icl_temp["follow_ques"])})
            if self.qg_model_name == "qwen":
                text = self.qg_model["processor"].apply_chat_template(
                    messages, tokenize=False, add_generation_prompt=True
                )
            ...
            if self.qg_model_name == "llava":
                ...
                output = self.qg_model["model"].generate(
                    **inputs, max_new_tokens=512, do_sample=True, temperature=0.5
                )
                ques = (
                    self.qg_model["processor"]
                    .decode(output[0], skip_special_tokens=True, clean_up_tokenization_spaces=False)
    ...
    def gen_question(
        self, evid_context, claim_txt, claim_img, meta_date, meta_loc, num_orig_claim_imgs, qg_icl=False
    ):
        ...
        if qg_icl == False:
            if len(evid_context) == 0:
                ...
                ques = self.gen_follow_ques(
                    claim_txt, claim_img, evid_context, meta_date, meta_loc, num_orig_claim_imgs
                )
        # Few-shot question generation
        else:
            ques = self.gen_ques_few_shot(
                claim_txt, claim_img, evid_context, meta_date, meta_loc, num_orig_claim_imgs
            )
        return ques
        ...
        if qg_icl == False:
            ...
            if "gemini" in self.qg_model_name:
                ...
                if self.qg_model_name == "llava":
                    ...
                    output = self.qg_model["model"].generate(
                        **inputs, max_new_tokens=1024, do_sample=True, temperature=0.5
                    )
                    ques = (
                        self.qg_model["processor"]
                        .decode(output[0], skip_special_tokens=True, clean_up_tokenization_spaces=False)
