...
class QA_Model:
    ...
    def __init__(
        ...
        self.planner = (
            planner  # select answering method: reverse image search; texutal search; # image comparison
        )
    ...
    def tool_executor(
        self, claim_txt, claim_img, ques_txt, ques_type, img_idx, ans_method, meta_date, meta_loc, req_id
    ):
        evid_url = None  # initialization; for VQA, no evid urls
        if ans_method in ["B", "D"]:
            answer, detailed_evid, evid_url = self.tool_mapper[ans_method](
                ques_txt,
                img_idx,
                claim_txt,
                claim_img,
    ...
    def answer_ques(
        ...
        if prev_ques_info is not None:
            ...
            answer, detailed_evid, evid_url = self.tool_executor(
                claim_txt, claim_img, ques_txt, ques_type, img_idx, ans_method, meta_date, meta_loc, req_id
            )
        ...
        """
        evid = qa_to_evidence.qa_to_evid(
            evid_ques, evid_ans, self.answer_llm, self.answer_llm_name, self.path
        )
